...
export default function CartPage() {
  ...
  const shipping = subtotal >= FREE_SHIPPING_THRESHOLD ? 0 : 1000;
          ...
          <>
            ...
            <div className="w-[95%] sm:w-[90%] mx-auto bg-gradient-to-tl from-[#eff0f0] to-[#d9dbde] p-4 rounded shadow mb-6">
              <p className="text-sm font-medium mb-2 text-gray-700">
                {FREE_SHIPPING_THRESHOLD - subtotal > 0 ? `PKR ${(FREE_SHIPPING_THRESHOLD - subtotal).toLocaleString()} away from FREE SHIPPING!` : 'Eligible for FREE SHIPPING!'}
              </p>
                ...
                {cartItems.map(item => (
                  ...
                  >
                    <div className="flex items-center gap-4">
                      <Image
                        src={item.image?.startsWith('/') ? `${API_BASE_URL}${item.image}` : item.image || '/placeholder.png'}
                        alt={item.name}
                        width={70}
                        height={10}
                        className="rounded object-cover w-[4.5rem] h-[6rem]"
                      />
                      <div>
              ...
              <div className="bg-gradient-to-br from-[#eff0f0] to-[#d9dbde] rounded shadow p-6 h-fit">
                ...
                return (
                  <div className="space-y-2">
                    ...
                    <div className='flex justify-between'>
                      <div>Shipping:</div>
                      <div className={`font-semibold ${shipping === 0 ? 'text-green-600' : ''}`}>{shipping === 0 ? 'Free' : `${shipping.toLocaleString()} PKR`}</div>
                    </div>
                    <div className='flex justify-between'>
                    ...
                    <hr />
                    <div className='flex justify-between'>
                      <div>Total ({totalItems > 10 ? '20% off' : '0% off'}):</div>
                      <div className="font-semibold">{finalTotal.toLocaleString()} PKR</div>
                    </div>
                    <button
                      onClick={handleProceedToCheckout}
                      className="mt-4 w-full bg-[#ea580c] hover:bg-[#c2410c] text-white px-6 py-2 rounded transition-colors"
                    >
                      Proceed to Checkout
                    </button>
                  </div>
